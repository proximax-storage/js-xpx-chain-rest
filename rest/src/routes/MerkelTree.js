...
const {sha3Hasher} = catapult.crypto.sha3Hasher
...
class MerkleTree {
	...
	static getBranchHash(encodedPath, links) {
		...
		return catapult.utils.convert.uint8ToHex(sha3Hasher.calculate(catapult.utils.convert.hexToUint8(encodedPath + branchLinks.join('')), 32));
	}
	...
	static getLeafHash(encodedPath, leafValue) {
		return catapult.utils.convert.uint8ToHex(sha3Hasher.calculate(catapult.utils.convert.hexToUint8(encodedPath + leafValue), 32));
	}
}
