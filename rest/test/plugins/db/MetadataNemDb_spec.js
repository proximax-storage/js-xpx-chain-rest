...
describe('metadata db', () => {
	...
	describe('metadata', () => {
		...
		const testKey2 = address.stringToAddress('NAR3W7B4BCOZSZMFIZRYB3N5YGOUSWIYJCJ6HDAA');
		...
		const createMetadata = (objectId, sourceAddress, targetKey, scopedMetadataKey, targetId, metadataType) => ({
			_id: createObjectId(objectId),
			metadataEntry: {
				sourceAddress: sourceAddress ? Buffer.from(sourceAddress) : undefined,
				targetKey: targetKey ? Buffer.from(targetKey) : undefined,
		...
		it('returns filtered metadata by sourceAddress', () => {
			// Arrange:
			const dbMetadata = [
				createMetadata(10, testKey1),
				createMetadata(20, testKey2)
			];

			// Act + Assert:
			return runTestAndVerifyIds(
				dbMetadata,
				db => db.metadata(testKey2, undefined, undefined, undefined, undefined, paginationOptions), [20]
			);
		});

		it('returns filtered metadata by targetKey', () => {
			// Arrange:
			const dbMetadata = [
				createMetadata(10, undefined, testKey1),
				createMetadata(20, undefined, testKey2)
			];

			// Act + Assert:
			return runTestAndVerifyIds(
				dbMetadata,
				db => db.metadata(undefined, testKey2, undefined, undefined, undefined, paginationOptions), [20]
			);
		});
