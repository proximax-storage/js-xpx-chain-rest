...
describe('aggregate routes', () => {
	...
	describe('PUT transaction cosignature', () => {
		test.route.packet.addPutPacketRouteTests(aggregateRoutes.register, {
			...
			inputs: {
				valid: {
					params: { signer: '123456', signature: '998811', scheme: '01', parentHash: 'ABEF' },
					parsed: Buffer.of(
						0x11, 0x00, 0x00, 0x00, // size (header)
						0xF5, 0x01, 0x00, 0x00, // type (header)
						0x12, 0x34, 0x56, 0x99, 0x88, 0x11, 0x01, 0xAB, 0xEF // payload (signer, signature, parentHash)
					)
				},
				invalid: {
					params: { signer: '123456', signature: '998S11', scheme: 'SS',  parentHash: 'ABEF' },
					error: { key: 'signature' }
				}
			}
		});
