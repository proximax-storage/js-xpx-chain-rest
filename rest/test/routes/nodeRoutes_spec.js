...
describe('node routes', () => {
	describe('get', () => {
		const serviceCreator = function(packets) {
			let i = 0;
			const service = {
				connections: {
					singleUse: () => new Promise(resolve => {
						resolve({
							pushPull: () => new Promise(innerResolve => innerResolve(packets[i++]))
						});
					})
				},
				config: {
		...
		it('can retrieve nodes information', () => {
			// Arrange:
			const nodeInfosPacket = {
				type: 603,
				size: 49 * 2,
				payload: Buffer.from([
					// Empty fields node
					0x31, 0x00, 0x00, 0x00, 0xE3, 0x27, 0xC0, 0xF1, 0xC9, 0x97, 0x5C, 0x3A, 0xA5, 0x1B, 0x2A, 0x41,
					0x76, 0x81, 0x58, 0xC1, 0x07, 0x7D, 0x16, 0xB4, 0x60, 0x99, 0x9A, 0xAB, 0xE7, 0xAD, 0xB5, 0x26,
					0x2B, 0xE2, 0x9A, 0x68, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
					0x00,
					// correct node
					0x31, 0x00, 0x00, 0x00, 0xE3, 0x27, 0xC0, 0xF1, 0xC9, 0x97, 0x5C, 0x3A, 0xA5, 0x1B, 0x2A, 0x41,
					0x76, 0x81, 0x58, 0xC1, 0x07, 0x7D, 0x16, 0xB4, 0x60, 0x99, 0x9A, 0xAB, 0xE7, 0xAD, 0xB5, 0x26,
			...
			const currentNodePacket = {
				type: 601,
				size: 57,
				payload: Buffer.from([
					0x31, 0x00, 0x00, 0x00, 0xAA, 0xBB, 0xDD, 0xCC, 0xC9, 0x97, 0x5C, 0x3A, 0xA5, 0x1B, 0x2A, 0x41,
					0x76, 0x81, 0x58, 0xC1, 0x07, 0x7D, 0x16, 0xB4, 0x60, 0x99, 0x9A, 0xAB, 0xE7, 0xAD, 0xB5, 0x26,
					0x2B, 0xE2, 0x9A, 0x68, 0xDC, 0x1E, 0x90, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00,
					0x00
				])
			};
			const services = serviceCreator([nodeInfosPacket, currentNodePacket]);
				...
				routeContext.routeInvoker().then(() => {
					...
					expect(routeContext.responses[0]).to.deep.equal({
						...
						payload: [{
							...
							networkIdentifier: 144,
							port: 7900,
							publicKey: Buffer.from(convert.hexToUint8('AABBDDCCC9975C3AA51B2A41768158C1077D16B460999AABE7ADB5262BE29A68')),
							roles: 2,
							version: 0
						}, {
							friendlyName: Buffer.alloc(0),
